import "mocha";
import { addedItem, dispatchedItem, removedItem, selectOrdersLength, assignItemOwner, selectOrdersByOwner } from "../src/store/orders";
...
describe('Store', function () {
    it("dispatches the event", () => {
        ...
        assert.strictEqual(selectOrdersByOwner("Bob")(store.getState()).length, 0);
        ...
        assert.strictEqual(selectOrdersByOwner("Bob")(store.getState()).length, 1);
        store.dispatch(assignItemOwner({ id: 1, owner: "Bob" }));
        assert.strictEqual(selectOrdersByOwner("Bob")(store.getState()).length, 2);
